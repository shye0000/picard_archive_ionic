...
.controller('AppCtrl', function($scope, $ionicModal, $timeout, $http, $rootScope) {
  ...
  $scope.login = function() {
    //var ref = window.open('http://localhost:9292/idoc-picard.ias.u-psud.fr/sitools/authentication/login-mandatory', '_system', 'location=no');
    $scope.modal.show();
  };
  ...
  $scope.doLogin = function() {
    ...
    var loginUrl='http://localhost:9292/idoc-picard.ias.u-psud.fr/sitools/authentication/login';
    $http({
      method: 'GET',
      url: loginUrl
...
.controller('UserCtrl', function($scope, $http, $rootScope) {
  $http.get('http://localhost:9292/idoc-picard.ias.u-psud.fr/sitools/editProfile/'+$rootScope.userId).then(function(resp) {
...
.controller('dseCtrl', function($scope, $http, $ionicActionSheet, $rootScope) {
  ...
  $http.get('http://localhost:9292/idoc-picard.ias.u-psud.fr/aa6baaff-87c1-4298-948a-5dc72c94e457/graph?media=json').then(function(resp) {
...
.controller('downloadAsynCtrl', function($scope, $http, $stateParams) {
  ...
  $scope.download = function(filename){
    $scope.busy=true;
    var downloadUrl='http://localhost:9292/idoc-picard.ias.u-psud.fr/picard/plugin?1=1&colModel=%22filename,%20dir_fits,%20instrume,%20telescop,%20datetimeobs,%20obs_mode,%20obs_type,%20level_pro,%20lambda,%20exposure,%20noimage,%20xcenter,%20ycenter,%20obs_alt,%20dsun,%20earth_d,%20obs_cr%22&p[0]=LISTBOXMULTIPLE|id';
...
.controller('DatatableCtrl', function($scope, $ionicModal, $http, $stateParams, $timeout, $ionicActionSheet) {
  ...
  $scope.loadData = function(){
    $scope.busy=true;
    var url='http://localhost:9292/idoc-picard.ias.u-psud.fr/'+$stateParams.url+'/records?nocount=false&start='+$scope.starter+'&limit=50&media=json';
...
//alert($stateParams.url);
  ...
  $scope.loadData = function(){
    $scope.busy = true;
    var paracount=0;
    var url='http://localhost:9292/idoc-picard.ias.u-psud.fr/'+$stateParams.url+'/records?nocount=false&start='+$scope.starter+'&limit=50';
...
.controller('qfCtrl', function($scope, $http, $ionicActionSheet) {
  ...
  $http.get('http://localhost:9292/idoc-picard.ias.u-psud.fr/aa6baaff-87c1-4298-948a-5dc72c94e457/forms').then(function(resp) {
...
.controller('ptCtrl', function($scope, $http) {
  console.log($scope);
  $scope.busy = true;
  $http.get('http://localhost:9292/idoc-picard.ias.u-psud.fr/sitools/upload/timeline_picard.json').then(function(resp) {
